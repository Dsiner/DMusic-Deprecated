...
public class MusicControlUtils {
    ...
    private int playStatus;//0:无 1:播放 2:暂停
     ...
     * 播放/暂停
     *
     * @return:执行结果0:无音乐 1:播放 2:暂停
     */
    public int playOrPause() {
        if (isLoaded) {
            PlayOrPauseEvent popEvent = new PlayOrPauseEvent();
            if (mediaPlayer.isPlaying()) {
                mediaPlayer.pause();
                playStatus = 2;//暂停
                popEvent.isPlay = false;
            } else {
                mediaPlayer.start();
                playStatus = 1;//正在播放
                popEvent.isPlay = true;
            }
     ...
     * @return
     */
    public MusicListModel getCurrentMusicListModel() {
        if (musicListModels != null && musicListModels.size() > 0) {
            return musicListModels.get(currentLocation);
        }
        return null;
    }
